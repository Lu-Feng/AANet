...
class TripletsDataset(BaseDataset):
    ...
    def get_best_positive_globalRanking(self, args, query_index, cache, query_features):
        ...
        _, best_positive_num = faiss_index.search(query_features.reshape(1, -1), positives_features.shape[0])
        return best_positive_num

    def get_best_positive_localRanking(self, query_index, cache_a):
        positives_features_a = cache_a[self.hard_positives_per_query[query_index]]
        query_features_a = cache_a[query_index + self.database_num]
    ...
    def get_best_positive_index(self, args, query_index, cache, cache_a, query_features):
        # # print("---------")
        # # print(self.queries_paths[query_index])
        # # for i in self.hard_positives_per_query[query_index]:
        ...
        sort1 = self.get_best_positive_globalRanking(args, query_index, cache, query_features)[0]
        sort2 = self.get_best_positive_localRanking(query_index, cache_a)
        # print("-----------------------------------------")
        # print("sort1",sort1)
        # print("sort2",sort2)
        sort2 = list(sort2)
        max_dif = 0
        best_positive_num = sort1[0]
        n = len(sort1)
        if 0<args.candipositive_global<1:  # for msls
            for i in range(n):
                if i <= int(args.candipositive_global*n) or sort2.index(sort1[i]) <= int(args.candipositive_local*n):
                    if abs(i-sort2.index(sort1[i]))> max_dif:
                        max_dif = abs(i-sort2.index(sort1[i]))
                        best_positive_num = sort1[i]
        else:  # for pitts30k
            for i in range(n):
                if i < args.candipositive_global or sort2.index(sort1[i]) < args.candipositive_local:
                    if abs(i-sort2.index(sort1[i]))> max_dif:
                        max_dif = abs(i-sort2.index(sort1[i]))
                        best_positive_num = sort1[i]
        best_positive_index = self.hard_positives_per_query[query_index][best_positive_num].item()
    ...
    def compute_triplets_full(self, args, model):
        ...
        for query_index in tqdm(sampled_queries_indexes, ncols=100):
            query_features = self.get_query_features(query_index, cache)
            #best_positive_index = self.get_best_positive_index(args, query_index, cache, query_features)
            best_positive_index = self.get_best_positive_index(args, query_index, cache, cache_a, query_features)
